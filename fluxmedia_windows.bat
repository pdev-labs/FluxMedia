...
:: 0. Extract embedded python script to a temp file
set BAT_PATH=%~f0
set TEMP_PY=%TEMP%\fluxmedia_aio_%RANDOM%.py
powershell -NoProfile -Command "$txt = Get-Content -Raw -LiteralPath $env:BAT_PATH; $split = $txt -split '(?m)^:: ===PYTHON_START===\r?\n'; if ($split.Length -gt 1) { [System.IO.File]::WriteAllText($env:TEMP_PY, $split[1]) }"
...
if exist requirements.txt (
    ...
    python -m pip install "yt-dlp>=2025.2.18" "requests>=2.31.0" "rich>=13.7.0" >nul 2>&1
)
if errorlevel 1 (
...
:: 6. Clean up the temp python file
if exist "%TEMP_PY%" del "%TEMP_PY%"
