        ...
        class SilentHandler(SimpleHTTPRequestHandler):
            ...
            def do_POST(self):
                ...
                if parsed_url.path == "/api/sync/ping":
                    ...
                    try:
                        ...
                        if client_id:
                            SYNC_STATE["clients"].add(client_id)
                    except Exception:
                        pass
                    self.send_response(200)
                    self.send_header("Access-Control-Allow-Origin", "*")
            ...
            def do_GET(self):
                ...
                if path == "/api/sync/state":
                    ...
                    state_copy = SYNC_STATE.copy()
                    state_copy["clients_count"] = len(SYNC_STATE["clients"])
                    state_copy.pop("clients", None)
...
def operation_sync_play(config: Dict[str, Any]):
    ...
    try:
        r = requests.get(f"{server_url}/api/sync/state", timeout=2)
    ...
    if not server_running:
        ...
        for _ in range(5):
            time.sleep(1)
            try:
                r = requests.get(f"{server_url}/api/sync/state", timeout=1)
                if r.status_code == 200:
                    server_running = True
                    break
            except requests.exceptions.RequestException:
        ...
        if not server_running:
            console.print("[bold red]Failed to start Share Server.[/bold red]")
            Prompt.ask("\nPress Enter to return...")
            return
    ...
    SYNC_STATE["clients"].clear()
    SYNC_STATE["active"] = True
    
    try:
        while True:
            if len(SYNC_STATE["clients"]) > 0:
                break
            time.sleep(2)
    except KeyboardInterrupt:
    ...
    console.print(f"[green]Found {len(SYNC_STATE['clients'])} connected device(s)![/green]")
    ...
    file_choice = Prompt.ask("Select media file to play (No.)", choices=[str(i+1) for i in range(len(files))] + ["q"])
    if file_choice.lower() == 'q':
        SYNC_STATE["active"] = False
        return
        
    file_idx = int(file_choice) - 1
    media_file = files[file_idx]
    
    console.print(f"\n[bold green]Loading '{media_file}' on devices...[/bold green]")
    ...
    while True:
        ...
        if cmd == 'q':
            ...
            SYNC_STATE["last_update"] = time.time()
            console.print(f"[green]State: {SYNC_STATE['state']}[/green]")
        elif cmd == 's':
            try:
